...
/**
 * HBase 工具类
 * Created by jiangyun on 2017/12/16.
 */
    ...
    static {
        try {
            if (conf == null) {
                ...
                conf.set("hbase.zookeeper.quorum", "cors-01,cors-02,cors-03");
...
//         System.out.println("---------- " + conn.hashCode());
    ...
    private static void createTable(String tableName, String[] cfs) throws Exception {
        ...
        try {
            if (admin.tableExists(tableName)) {
                logger.warn("Table: {} is exists!", tableName);
                System.out.println("Table: {} is exists!" + tableName);
                return;
            }
            HTableDescriptor tableDesc = new HTableDescriptor(TableName.valueOf(tableName));
            for (int i = 0; i < cfs.length; i++) {
...
//				conn.close();
    ...
    public static Put objectToPut(Object object, String tableName, String cf) throws IllegalAccessException {
